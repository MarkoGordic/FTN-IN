...
#pragma omp parallel firstprivate(zaradaPoKuriru, normaPaketaPoKuriru)
    {
// postavljanje tezine paketa, zaduzenje jednog kurira
#pragma omp single
        {
            for (int i = 0; i < BROJ_PAKETA; i++) {
                // ista logika za postavljanje tezine paketa i u paralelnom
                // algoritmu treba da bude iskoristena
                tezinePaketa[i] = rand() % MAKS_TEZINA_PAKETA + 1;
...
#pragma omp for schedule(static, brojPaketaPoKuriru)
        for (int i = 0; i < BROJ_PAKETA; i++) {
            // određivanje id-a trenutnog radnika
            int trenutniKurir = omp_get_thread_num();
...
#pragma omp critical
            {
                if (zaradaPoKuriru > uk.ukupnaZarada) {
                    uk.ukupnaZarada = zaradaPoKuriru;
                    uk.idKurira = omp_get_thread_num();
                }
            }
        }
    } // kraj parallel regiona
    printf("\n\n");
