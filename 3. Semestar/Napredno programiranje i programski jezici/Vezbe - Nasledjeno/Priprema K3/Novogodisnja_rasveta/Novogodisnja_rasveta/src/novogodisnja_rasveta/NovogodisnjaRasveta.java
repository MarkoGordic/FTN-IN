...
public class NovogodisnjaRasveta {
	private ArrayList<Lampion> lampioni; // inicijalizacija moze ili ovde ili u konstrukotru
	...
	public Lampion pronadji(double cena, String boja) {
		for (Lampion lamp : lampioni) {
			if (lamp.getCena() > cena && lamp.getBoja().equals(boja)) { // mora equals poredimo sadrzaj stringova, ne adrese
				return lamp;
			}
		}
		return null;
	...
	public boolean postaviAkciju() {
		...
		try {
			...
			System.out.println("Neka cudna greska!");
			return false;
		} finally {
			try {
				br.close();
			} catch (Exception e2) {
				System.out.println("Greska prilikom zatvaranja!");
			}
		}
		
		Lampion max = lampioni.get(0);
		for (Lampion lamp : lampioni) {
			if (lamp.getCena() > max.getCena()) {
				max = lamp;
			}
		}
		...
		sc.close();
		try {
			int cena = Integer.parseInt(str);
			potraga za maks dalje ide i onda na kraju ta provera
			ako sve propadne
			return false;
		} catch (NumberFormatException nfe) {
			return false;
	...
	public String toString() {
		String str = "**** NovogodisnjaRasveta ****\n";
		if (lampioni.isEmpty()) {
			str += "Nema lampiona!";
			return str;
		}
