...
parameter
  ...
  | _TYPE _ID
      ...
      {
        if($1 == VOID)
          err("parameter can not be void type");
      }
  ;
...
variable
  : _TYPE _ID _SEMICOLON
      ...
      {
        if($1 == VOID)
          err("variable can not be void type");
      }
  ;
...
branch_statement
  : _BRANCH _LPAREN _ID
   ...
   {
      int i = lookup_symbol($3, VAR|PAR);
      if(get_type(i) != get_type($9))
        err("incompatible types...");
      
      int pom = atoi(get_name($9));
      if(pom < branch_pom)
        err("literal 2 is smaller then first");
      else
        branch_pom = pom;
   }
   _COMMA literal
   {
      int i = lookup_symbol($3, VAR|PAR);
      if(get_type(i) != get_type($12))
        err("incompatible types...");
      
      int pom = atoi(get_name($12));
      if(pom < branch_pom)
        err("literal 3 is smaller then second");
      else
        branch_pom = pom;
   }
